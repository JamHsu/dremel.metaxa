...
public class CompilerTest {
	...
	public void testExpression() throws RecognitionException {
		...
		ExpNode exp = ExpNodeFactory.buildNode((AstNode) node.getChild(0).getChild(0), query);

		assertTrue(exp instanceof BinaryOp); // subtract
		BinaryOp bin1 = (BinaryOp) exp;
		assertTrue(bin1.getOperation().equals("-"));
		assertTrue(exp.getChild(0) instanceof BinaryOp); // add
		BinaryOp bin2 = (BinaryOp) exp.getChild(0);
		assertTrue(bin2.getOperation().equals("+"));
		assertTrue(exp.getChild(1) instanceof Function);
		assertTrue(((Function) exp.getChild(1)).getFunctionName().equals("length"));
	...
	public void testQuery1() throws RecognitionException {
		...
		assertTrue(((BinaryOp) query.getFilter().getRoot()).getOperation().equals("&&"));
	...
	public void testCompiler1() throws RecognitionException {
		...
		MetaxaQuery query = new MetaxaQuery(nodes);
		Compiler compiler = new dremel.compiler.impl.CompilerImpl();
	...
	public void testCompiler3() throws RecognitionException {
		AstNode nodes = Parser.parseBql("SELECT abc FROM [document];"); // abc is not a field name of Document
		MetaxaQuery query = new MetaxaQuery(nodes);
		Compiler compiler = new dremel.compiler.impl.CompilerImpl();
		compiler.analyse(query);
		assertTrue(query.getSelectExpressions().get(0).getRoot() instanceof Symbol);
	...
	public void testJaninoScript() throws Exception {
		...
		compiler.analyse(query);
		String code = compiler.generateCode(query);
		//System.out.println(code);
		IScriptEvaluator se = compiler.createScript(code);
		...
		assertTrue(outSlice.getValue(1) == inSlice.getValue(2));
		assertTrue(outSlice.getValue(2) == inSlice.getValue(2) + inSlice.getValue(1));
		...
		se.evaluate(new Object[] { inSlice, outSlice, context1 });
		assertTrue(outSlice.getValue(0) == 0);
		assertTrue(outSlice.getValue(1) == 0);
		assertTrue(outSlice.getValue(2) == 0);
		...
		se.evaluate(new Object[] { inSlice, outSlice, context1 });
		assertTrue(outSlice.getValue(0) == 0);
		assertTrue(outSlice.getValue(1) == 100);
		assertTrue(outSlice.getValue(2) == 105);
	}
